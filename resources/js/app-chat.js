...
document.addEventListener("DOMContentLoaded",()=>{
    ...
    s=(e,a,t)=>{
        ...
        let c=0;e.forEach(e=>{
            var t=e.textContent.toLowerCase().includes(a);e.classList.toggle("d-flex",t),
            e.classList.toggle("d-none",!t),t&&c++
        }
    ),
...
o(),c.chatUserStatus.forEach(e=>{
    e.addEventListener("click",()=>{
        var t;t=e.value,[document.querySelector(".chat-sidebar-left-user .avatar"),
            document.querySelector(".app-chat-contacts .avatar")].forEach(e=>{
...
c.chatContactListItems.forEach(e=>{
    e.addEventListener("click",()=>{
        c.chatContactListItems.forEach(
            e=>e.classList.remove("active")),
            e.classList.add("active"),t()}
        )
    }
),
c.searchInput?.addEventListener("keyup",debounce(e=>{e=e.target.value.toLowerCase();
...
c.formSendMessage?.addEventListener("submit",e=>{
    ...
    e&&((t=document.createElement("div")).className="chat-message-text mt-2",t.innerHTML=`<p class="mb-0 text-break">${e}</p>`,
    document.querySelector("li:last-child .chat-message-wrapper")?.appendChild(t),c.messageInput.value="",o()
)
}),
